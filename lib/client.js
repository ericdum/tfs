...
Client.prototype._init = function () {
  ...
  // 十分钟更新一次 servers
  setInterval(that.refreshServers.bind(that), 10 * 60 * 1000);
};
...
Client.prototype._request = function (cmd, callback) {
  ...
  urllib.request(options, args, function (err, data, res) {
    ...
    if (status === 500 && data) {
      ...
      title = title && title[1] || 'Internal Server Error';
      var serverName = /<td>server:<\/td>[^<]+<td>([^<]+)<\/td>/i.exec(data);
      serverName = serverName && serverName[1] || 'unknow server';
      err = new Error('TFS Server error: ' + title + ' (' + serverName + ')');
      err.name = 'TFSServerError';
    }
...
function getContent(file, callback) {
  ...
  if (file instanceof Stream) {
    var datas = [];
    file.on('data', datas.push.bind(datas));
    file.once('end', function () {
      callback(null, Buffer.concat(datas));
    });
    return;
  }
}
...
Client.prototype.upload = function (filepath, extname, callback, timeout) {
  ...
  var that = this;
  getContent(filepath, function (err, buffer) {
    if (err) {
      return callback(err);
    }
    ...
    if (Buffer.isBuffer(filepath)) {
      filepath = '_buffer';
      if (!extname) {
        // try to guess content-type and extention
        var r = bt(buffer);
        if (r) {
          extname = r.extension;
        }
...
Client.prototype.uploadPrivate = function (filename, extname, callback, timeout) {
  if (typeof extname === 'function') {
    timeout = callback;
    callback = extname;
    extname = null;
  }
  if (!extname && typeof filename === 'string') {
  ...
  this.upload(filename, extname, callback, timeout);
};
...
Client.prototype.getMeta = function (name, options, callback, timeout) {
  ...
  var cmd = this.createCommand('GetMeta', args);
  this._request(cmd, function (err, data) {
    if (err) {
      return callback(err);
    }
...
/**
 * v2 uploadFile, downloadFile, removeFile, getFileMeta, uploadPrivateFile
 */
...
Client.prototype.getFileMeta = function (uid, filename, callback, timeout) {
  ...
  var cmd = this.createCommand('GetFileMeta', args);
  this._request(cmd, function (err, data) {
    if (err) {
      return callback(err);
    }
...
Client.prototype.uploadFile = function (filepath, uid, filename, options, callback, timeout) {
  ...
  getContent(filepath, ep.done('content'));
  that.createFile(uid, filename, ep.done('createFile'), timeout);
};

/**
 * Upload a file with custom filename of private.
 *
 * @param {String} filepath
 * @param {String} uid, user id
 * @param {String} filename
 ...
 * @param {Number} [timeout]
 */
Client.prototype.uploadPrivateFile = function (filepath, uid, filename, options, callback, timeout) {
  if (typeof options === 'function') {
    timeout = callback;
    callback = options;
    options = null;
  ...
  if( filename.indexOf('tfsprivate') == -1 ) {
    filename = filename.replace(/(\.)?([^.]+)$/, '$1tfsprivate.$2')
  }

  var that = this;
  var privateCallback = function(err, info){
    if( err ) return callback(err, info);
    info[ 'name' ] = that.appid + '/' + uid + '/file/' + filename;
    info[ 'url'  ] = 'http://' + that.rootServerHost + ':' + that.servers[0][1] + '/v2/' + that.appkey + '/' + info[ 'name' ];
    callback( err, info )
  }

  this.uploadFile(filepath, uid, filename, options, privateCallback, timeout);
};
...
Client.prototype.downloadFile = function (uid, filename, savefile, options, callback, timeout) {
  ...
  var cmd = this.createCommand('DownloadFile', args);
  this._request(cmd, function (err) {
    if (err) {
      return callback(err);
    }
