...
UploadCommand.prototype._getURL = function () {
  ...
  var extname = this._args.extname || path.extname(filepath);
  extname = extname && extname[0] !== '.' ? '.' + extname : extname;
  if (extname) {
...
function GetMetaCommand(client, args) {
  GetMetaCommand.super_.call(this, client, args);
}
util.inherits(GetMetaCommand, Command);

GetMetaCommand.prototype._getURL = function () {
  var url = '/v1/' + this._client.appkey + '/metadata/' + this._args.name;
  var type = parseInt(this._args.type, 10);
  if (!isNaN(type)) {
    url += '?type=' + type;
  }
  return url;
...
GetMetaCommand.prototype._getArgs = function () {
  return {
    type: 'GET',
  };
};
...
/**
 * v2: createFile, uploadFile, removeFile, downloadFile
 */
...
UploadFileCommand.prototype._getURL = function () {
  ...
  if (typeof this._args.offset === 'number') {
...
function DownloadFileCommand(client, args) {
  DownloadFileCommand.super_.call(this, client, args);
}
util.inherits(DownloadFileCommand, Command);

DownloadFileCommand.prototype._getArgs = function () {
  var data = {};
  if (this._args.offset) {
    data.offset = this._args.offset;
  }
  if (this._args.size) {
    data.size = this._args.size;
  }
  return {
    type: 'GET',
    data: data,
...
function GetFileMetaCommand(client, args) {
  GetFileMetaCommand.super_.call(this, client, args);
}
util.inherits(GetFileMetaCommand, Command);

GetFileMetaCommand.prototype._getURL = function () {
  var url = '/v2/' + this._client.appkey + '/metadata/' + this._client.appid + '/' +
    this._args.uid + '/file/' + this._args.filename;
  return url;
};

GetFileMetaCommand.prototype._getArgs = function () {
  return {
    type: 'GET',
  };
};
...
exports.GetMetaCommand = GetMetaCommand;
...
exports.GetFileMetaCommand = GetFileMetaCommand;
